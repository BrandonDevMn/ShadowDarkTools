/**
 * Render functions for GM Tools sections of the Library tab.
 */
...
export function renderRunningTheGameSection(container) {
  container.appendChild(makeSectionHeader('Difficulty Classes'));
  container.appendChild(makeParagraph('Easy (9), Normal (12), Hard (15), Extreme (18). When you ask for a check, state the type: e.g. "easy Charisma check" or "DC 15 Dexterity check."'));

  container.appendChild(makeSectionHeader('Random Encounters'));
  const encList = makeReferenceList();
  encList.appendChild(makeReferenceCard('Check',          '1d6',   'Encounter occurs on a result of 1. Check at intervals per danger level.'));
  encList.appendChild(makeReferenceCard('Starting Distance', '1d6', '1: Close · 2–4: Near · 5–6: Far'));
  encList.appendChild(makeReferenceCard('Activity',       '2d6',   '2–4: Hunting · 5–6: Eating · 7–8: Building/nesting · 9–10: Socializing · 11: Guarding · 12: Sleeping'));
  encList.appendChild(makeReferenceCard('Reaction Check', '2d6 + CHA', '0–6: Hostile · 7–8: Suspicious · 9: Neutral · 10–11: Curious · 12+: Friendly'));
  container.appendChild(encList);

  container.appendChild(makeSectionHeader('Traps (d12)'));
  ...
  container.appendChild(makeSectionHeader('Hazards (d12)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Awarding XP'));
  container.appendChild(makeTable(
    ['Quality', 'XP', 'Examples'],
    [
  ...
  container.appendChild(makeSectionHeader('Gold per Treasure Find'));
  container.appendChild(makeTable(
  ...
  const modes = makeReferenceList();
  modes.appendChild(makeReferenceCard('Hunter Mode',   null, 'Defeated monsters grant XP equal to half their level (round down).'));
  modes.appendChild(makeReferenceCard('Momentum Mode', null, 'Advantage on checks to repeat a failed action; damage dice explode (no cap).'));
  modes.appendChild(makeReferenceCard('Pulp Mode',     null, 'No luck token max; start each session with 1d4 tokens; tokens convert hits to crits or grant extra actions.'));
  modes.appendChild(makeReferenceCard('Blitz Mode',    null, 'Light timers last 30 minutes.'));
  modes.appendChild(makeReferenceCard('Chaos Mode',    null, 'Reroll initiative at the start of every combat round.'));
  modes.appendChild(makeReferenceCard('Deadly Mode',   null, 'Death timer is always 1; DC 18 INT to stabilize.'));
  modes.appendChild(makeReferenceCard('Fatality Mode', null, 'Characters die at 0 hit points.'));
  modes.appendChild(makeReferenceCard('Grinder Mode',  null, 'Regain only 1 stat damage per stat and one hit die worth of HP per rest; spellcasters recover only 1d4 lost spells.'));
  container.appendChild(modes);

  container.appendChild(makeSectionHeader('Dungeon Maps — Dice Method'));
  container.appendChild(makeTable(
    ['d6', 'Danger Level'],
  ...
  container.appendChild(makeSectionHeader('Room Contents (d10)'));
  container.appendChild(makeTable(
    ['d10', 'Feature'],
    [
...
export function renderMonstersSection(container) {
  container.appendChild(makeSectionHeader('Stat Block Format'));
  container.appendChild(makeParagraph('AC · HP · ATK (attacks, range, bonus, damage) · MV (movement) · S/D/C/I/W/Ch (stat mods) · AL (alignment) · LV (level). Followed by any special talents.'));

  container.appendChild(makeSectionHeader('Monster Rules'));
  ...
  container.appendChild(makeSectionHeader('Combat Roles'));
  const roles = makeReferenceList();
  roles.appendChild(makeReferenceCard('Mook',       null, 'Low damage, HP, AC, and attack bonus. Swarmy.'));
  roles.appendChild(makeReferenceCard('Soldier',    null, 'Mid-range damage, HP, AC, and attack bonus.'));
  roles.appendChild(makeReferenceCard('Striker',    null, 'High damage and attack bonus. Low HP and AC. Often stealthy.'));
  roles.appendChild(makeReferenceCard('Tank',       null, 'Mid-range damage and attack bonus. High HP and AC.'));
  roles.appendChild(makeReferenceCard('Controller', null, 'Environment-altering talents. Mid-range damage, low HP and AC.'));
  roles.appendChild(makeReferenceCard('Legendary',  null, 'High HP, AC, damage, and/or attack bonus.'));
  container.appendChild(roles);

  container.appendChild(makeSectionHeader('Level Tiers'));
  container.appendChild(makeTable(
    ['LV', 'Tier', 'Treasure'],
    [
  ...
  container.appendChild(makeSectionHeader('Damage by Level'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('1:1 Monster Difficulty'));
  container.appendChild(makeTable(
    ['Avg. Party Level', 'Monster LV per PC'],
    [['0–3', '1'], ['4–6', '3'], ['7–9', '5']]
  ));

  container.appendChild(makeSectionHeader('Monster Generator (d20)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Monster Mutations (d12 — up to 3)'));
  container.appendChild(makeTable(
...
export function renderMagicItemsSection(container) {
  container.appendChild(makeSectionHeader('Design Principles'));
  const tips = makeReferenceList();
  tips.appendChild(makeReferenceCard('Unique',       null, 'All magic items should be distinct. Give them personalities, strange curses, or unique appearances.'));
  tips.appendChild(makeReferenceCard('Power Level',  null, 'Benefits should each be roughly equivalent to a class talent. Magic items can be mighty, but rarely eclipse what a character contributes.'));
  tips.appendChild(makeReferenceCard('Darkvision Caution', null, 'Be very careful with effects that remove the need for torches or the light spell — they remove core time pressure.'));
  tips.appendChild(makeReferenceCard('Gear Slots',   null, 'Enhancing gear slots reduces core challenges. Such effects should be rare and limited.'));
  container.appendChild(tips);

  container.appendChild(makeSectionHeader('Qualities Table (2d6)'));
  container.appendChild(makeTable(
    ['2d6', 'Benefits', 'Curses'],
    [['2–3', '—', '1'], ['4–7', '1', '1'], ['8–11', '1', '—'], ['12', '2', '—']]
  ));

  container.appendChild(makeSectionHeader('Personality Table (2d6)'));
  container.appendChild(makeTable(
    ['2d6', 'Virtues', 'Flaws'],
    [['2–3', '—', '1'], ['4–9', '—', '—'], ['10–11', '1', '1'], ['12', '1', '—']]
  ));

  container.appendChild(makeSectionHeader('Item Type (d6)'));
  container.appendChild(makeTable(
    ['d6', 'Item'],
    [['1', 'Armor'], ['2', 'Potion'], ['3', 'Scroll'], ['4', 'Utility'], ['5', 'Wand'], ['6', 'Weapon']]
  ));

  container.appendChild(makeSectionHeader('Bonus'));
  container.appendChild(makeParagraph('+0 and +1 items are most frequent. +3 items are very rare. Armor bonus adds to AC. Weapon bonus adds to attack and damage rolls.'));

  container.appendChild(makeSectionHeader('Magic Item Values'));
  container.appendChild(makeParagraph('Magic items are rarely bought or sold. When they are: weak items cost 1d6 × 100 gp; powerful items cost 2d6 × 100 gp (or are priceless).'));
}

// ── Treasure ───────────────────────────────────────────────────────────────

export function renderTreasureSection(container) {
  container.appendChild(makeSectionHeader('XP for Treasure Quality'));
  container.appendChild(makeTable(
    ['Quality', 'XP', 'Examples'],
    [
      ['Poor',      '0',  'Bag of silver, used dagger, knucklebone dice'],
  ...
  container.appendChild(makeSectionHeader('Boons'));
  const boons = makeReferenceList();
  boons.appendChild(makeReferenceCard('Oaths',    null, 'A favor owed — a unicorn\'s promise, a baron\'s title, a dryad\'s safe harbor.'));
  boons.appendChild(makeReferenceCard('Secrets',  null, 'The identity of a murderer, an incriminating letter, or the True Name of a demon.'));
  boons.appendChild(makeReferenceCard('Blessings',null, 'A magical enchantment from a fountain or deity gift. Can be temporary or permanent.'));
  container.appendChild(boons);

  container.appendChild(makeSectionHeader('Sample Oaths (d8)'));
  container.appendChild(makeTable(
    ['d8', 'Oath'],
    [
  ...
  container.appendChild(makeSectionHeader('Sample Secrets (d12)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Sample Blessings (d12)'));
  container.appendChild(makeTable(
    ['d12', 'Blessing', 'Effect'],
    [
...
export function renderNpcsAndReactionsSection(container) {
  container.appendChild(makeSectionHeader('Reaction Check (2d6 + CHA mod)'));
  container.appendChild(makeTable(
    ['Roll', 'Attitude'],
    [
      ['0–6',  'Hostile'],
  ...
  container.appendChild(makeSectionHeader('Random NPC'));
  container.appendChild(makeTable(
    ['Table', 'Die', 'Options'],
    [
  ...
  container.appendChild(makeSectionHeader('NPC Qualities (d20)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Occupation (d4, d4)'));
  container.appendChild(makeTable(
    ['d4,d4', '1', '2', '3', '4'],
    [
  ...
  container.appendChild(makeSectionHeader('NPC Epithet (d4, d4)'));
  container.appendChild(makeTable(
    ['d4,d4', '1', '2', '3', '4'],
    [
  ...
  container.appendChild(makeSectionHeader('Rival Crawlers'));
  const rivals = makeReferenceList();
  rivals.appendChild(makeReferenceCard('Size', '1d4 + 1 members', 'All rivals share the same alignment.'));
  rivals.appendChild(makeReferenceCard('Level', '1d6 per member', 'Roll 1d6 for each rival crawler\'s starting level.'));
  container.appendChild(rivals);
}

// ── Adventure Generator ────────────────────────────────────────────────────

export function renderAdventureGeneratorSection(container) {
  container.appendChild(makeParagraph('Roll some or all details and modify connecting words as needed. E.g. "Tower of the Deepwood Swamp" could be "Deepwood Swamp Tower."'));

  container.appendChild(makeSectionHeader('Adventure Generator (d20)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Site Name Generator (d20)'));
  container.appendChild(makeTable(
...
export function renderOverlandTravelSection(container) {
  container.appendChild(makeSectionHeader('Travel per Day'));
  container.appendChild(makeParagraph('PCs can travel up to 8 hours per day. Hexes are 6 miles across. Push further with CON checks.'));
  container.appendChild(makeTable(
    ['Method', 'Time per Hex'],
    [
      ['Walking',           '4 hours'],
  ...
  container.appendChild(makeSectionHeader('Navigation'));
  container.appendChild(makeParagraph('Navigator makes an INT check when exiting an unfamiliar hex. On failure, the group moves into a random adjacent hex.'));

  container.appendChild(makeSectionHeader('Food and Water'));
  container.appendChild(makeParagraph('PCs can go 3 days without a ration. After that: 1 CON damage per day (death at 0). Forage for 1 ration per day with an INT check.'));

  container.appendChild(makeSectionHeader('Hex Tables'));
  container.appendChild(makeTable(
    ['d6', 'Hex Danger Level'],
  ...
  container.appendChild(makeSectionHeader('Cataclysm (d8)'));
  container.appendChild(makeTable(
    ['d8', 'Type'],
    [
  ...
  container.appendChild(makeSectionHeader('Points of Interest (d20)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Settlement Names (d8)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Downtime'));
  container.appendChild(makeTable(
...
export function renderEncounterTablesSection(container) {
  container.appendChild(makeParagraph('Roll d100 and consult the matching terrain table. Select a terrain below.'));

  // Build terrain selector
  const selector = document.createElement('select');
  selector.className = 'reference-terrain-select';

  const defaultOpt = document.createElement('option');
  defaultOpt.value = '';
  defaultOpt.textContent = '— Choose terrain —';
  selector.appendChild(defaultOpt);

  TERRAIN_NAMES.forEach((name) => {
  ...
  tableContainer.className = 'reference-encounter-table';
  container.appendChild(tableContainer);

  selector.addEventListener('change', () => {
    tableContainer.innerHTML = '';
    const terrain = selector.value;
    if (!terrain || !ENCOUNTER_TABLES[terrain]) return;

    const tbl = makeTable(['d100', 'Encounter'], ENCOUNTER_TABLES[terrain]);
...
export function renderRandomEventsSection(container) {
  container.appendChild(makeParagraph('Roll on this table when you want an unexpected event to spice up a scene or moment of travel.'));
  container.appendChild(makeTable(['d100', 'Details'], RANDOM_EVENTS));
}
...
export function renderRumorsSection(container) {
  container.appendChild(makeParagraph('Roll or choose rumors that NPCs might share with the party.'));
  container.appendChild(makeTable(['d100', 'Rumor'], RUMORS));
}

// ── Settlements ────────────────────────────────────────────────────────────

export function renderSettlementsSection(container) {
  container.appendChild(makeSectionHeader('Settlement Type (d6)'));
  container.appendChild(makeTable(
    ['d6', 'Type', 'Dice'],
    [['1', 'Village', '3d4'], ['2–3', 'Town', '4d4'], ['4–5', 'City', '6d6'], ['6', 'Metropolis', '8d8']]
  ));

  container.appendChild(makeSectionHeader('Districts (d8)'));
  container.appendChild(makeTable(
    ['d8', 'Type'],
    [
  ...
  container.appendChild(makeSectionHeader('District Points of Interest (d6)'));
  const poiData = [
  ...
  container.appendChild(makeSectionHeader('Tavern Generator (d20)'));
  container.appendChild(makeTable(
    ['d20', 'Name', 'Known For'],
    [
  ...
  container.appendChild(makeSectionHeader('Food (d12)'));
  container.appendChild(makeTable(
  ...
  container.appendChild(makeSectionHeader('Drinks'));
  container.appendChild(makeTable(
    ['d*', 'Drink', 'Cost', 'Effect'],
    [
  ...
  container.appendChild(makeParagraph('Roll d6 for Poor taverns, 2d6 for Standard, d12 for Wealthy.'));

  container.appendChild(makeSectionHeader('Shop Generator (d20)'));
  container.appendChild(makeTable(
    ['d20', 'Name', 'Known For'],
    [
